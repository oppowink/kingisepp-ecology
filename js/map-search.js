...
(function () {
  ...
  function localAnswer(question) {
    ...
    if (/принять участ|участв|волонт[её]р|стать участник/.test(value)) {
      return 'Для участия нужен личный кабинет. После входа участник проходит обучение по методике, затем получает доступ к отправке наблюдения. Публикация на карте происходит только после проверки модератором.';
    }
    ...
    if (/после.*отправ|провер|модерац|одобр|отклон/.test(value)) {
      return 'После отправки наблюдение получает статус проверки. Модератор сверяет данные и фотографии. Только одобренное наблюдение может стать публичной точкой на карте; при отклонении участник видит причину.';
    }
  ...
  // запрос к AI (через прокси)
  async function askAi(question) {
    var response = await fetch((window.ECO_API_BASE || '') + '/api/ai/search', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({ question: question, context: window.workSummary || '' })
    });
    if (!response.ok) throw new Error('AI_NOT_AVAILABLE');
    var data = await response.json();
    if (!data.answer) throw new Error('AI_EMPTY');
    return data.answer;
  }
