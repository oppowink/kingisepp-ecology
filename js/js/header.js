...
(function () {
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    // ссылка «Модерация» в поднавигации кабинета
    try {
      var podMod = document.getElementById('podnavModerator');
      if (podMod) {
        var cachedPod = JSON.parse(sessionStorage.getItem('eco-preview-user-v1') || sessionStorage.getItem('eco-session-user-v1') || 'null');
        if (cachedPod && ['moderator', 'admin'].includes(cachedPod.role)) {
          podMod.hidden = false;
        }
      }
    } catch (_) {}
